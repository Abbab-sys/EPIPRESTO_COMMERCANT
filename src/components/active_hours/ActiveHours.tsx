...
import {
  DISPONIBILITY_CLOSING_HOUR_KEY,
  DISPONIBILITY_OPEN_HOUR_KEY,
...
/*
 * Name: Active Hours
 * Description: This component is used to display the active hours of a store.
 * Author: Alessandro van Reusel, Ryma Messedaa
 */
  ...
  return (
    <View>
      <View style={ActiveHoursStyles.flexDirectionRow}>
        <Text style={[SubTitleStyles.text, ActiveHoursStyles.subTitlesMargin]}>
          {translation('disponibility.days.' + props.day)}
        </Text>
        <IconButton
          style={[ActiveHoursStyles.iconSize, ActiveHoursStyles.plusIcon]}
          iconColor="white"
          icon="plus-thick"
          mode="contained"
          onPress={() => props.addActiveHour()}></IconButton>
      </View>
      <View style={ActiveHoursStyles.closedText}>
        {props.activeHours.length === 0 && (
          <Text>{translation('disponibility.closed')}</Text>
        )}
      </View>
      ...
      <ScrollView horizontal>
        {props.activeHours.map((activeHour, index) => {
          return (
            <View
              style={[
                ActiveHoursStyles.ActiveHoursView,
                ActiveHoursStyles.flexDirectionRow,
              ]}
              key={index}>
              <View style={ActiveHoursStyles.textInputView}>
                <TextInput
                  label={
                    <View style={ActiveHoursStyles.textInputLabelView}>
                      <Text style={ActiveHoursStyles.textInputLabel}>
                        {translation(DISPONIBILITY_OPEN_HOUR_KEY)}
                      </Text>
                    </View>
                  }
                ...
                <HelperText
                  type="error"
                  visible={activeHour.errorOpeningHour !== ''}>
                  {translation(
              ...
              <View style={ActiveHoursStyles.textInputView}>
                <TextInput
                  label={
                    <View style={ActiveHoursStyles.textInputLabelView}>
                      <Text style={ActiveHoursStyles.textInputLabel}>
                        {translation(DISPONIBILITY_CLOSING_HOUR_KEY)}
                      </Text>
                    </View>
                  }
                ...
                <HelperText
                  type="error"
                  visible={activeHour.errorEndingHour !== ''}>
                  {translation(
                ...
                {props.activeHours.length >= 0 && (
                  <IconButton
                    iconColor="white"
                    style={[
                      ActiveHoursStyles.minusIcon,
                      ActiveHoursStyles.iconSize,
                    ]}
                    icon="minus-thick"
                    mode="contained"
                    onPress={() => props.removeActiveHour(index)}></IconButton>
                )}
              </View>
            </View>
          );
