...
import { SafeAreaView, View, Text, Image, ScrollView, TouchableOpacity, Alert, ActivityIndicator } from 'react-native';
...
const OrderPage = ({ route, navigation }: any) => {
    ...
    const [current_order_status, set_current_order_status] = React.useState("WAITING_CONFIRMATION");
    ...
    const products: Product[] = orderObject.productsVariantsOrdered.map(({ relatedProductVariant, quantity }: any) => {
    ...
    const order: Order = {
        ...
        total: (orderObject.subTotal + orderObject.taxs + orderObject.deliveryFee).toFixed(2),
    ...
    const showSaveButton = () => {
        if (current_order_status !== order.logs[order.logs.length - 1].status && current_order_status !== "") {
            return (
